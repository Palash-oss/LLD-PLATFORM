...
All commands below should be executed inside the `lld-practice-platform/backend` directory.
...
Create or edit `.env` in `lld-practice-platform/backend`:
```env
PORT=3001
...
Commands to run the frontend application inside `lld-practice-platform/frontend`:
...
## 🔒 Security & Git Notice
No automated git pushes are performed by this repository configuration.
